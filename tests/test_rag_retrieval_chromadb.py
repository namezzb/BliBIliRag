...
class ChromaDBRetrieverTests(TestCase):
    def setUp(self) -> None:
        # 创建临时目录和 ChromaDB 向量存储
        self.temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp_dir.cleanup)
        base_path = Path(self.temp_dir.name)

        # 创建 ChromaDB 向量存储和 embedding provider
        self.vector_store = ChromaDBVectorStore(base_path / "chroma")
    ...
    def test_delete_by_bvid(self) -> None:
        """测试按 bvid 删除"""
        self.vector_store.delete_by_bvid("BV1A")
        count = self.vector_store.count()
        self.assertEqual(count, 1)


class ChromaDBMultiQueryRetrieverTests(TestCase):
    def setUp(self) -> None:
        # 创建临时目录和 ChromaDB 向量存储
        self.temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp_dir.cleanup)
...
class ChromaDBRAGChainTests(TestCase):
    def setUp(self) -> None:
        # 创建临时目录和 ChromaDB 向量存储
        self.temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp_dir.cleanup)
...
class ChromaDBRAGRetrievalServiceTests(TestCase):
    def setUp(self) -> None:
        # 创建临时目录和真实的数据库
        self.temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp_dir.cleanup)
        ...
        # 创建 ChromaDB 向量存储
        self.vector_store = ChromaDBVectorStore(self.settings.chroma_path)
    ...
    def test_vector_store_count(self) -> None:
        """测试向量存储计数"""
        count = self.vector_store.count()
        # 应该有 3 个文档（1 个视频级 + 1 个片段级 + 1 个视频级）
